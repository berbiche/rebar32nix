...
-define(REPLACE_TABS_WITH_SPACES(S),
    lists:flatten(string:replace(S, "\t", "\s\s\s\s\s\s\s\s", all))
).
...
get_deps_list(Filename) ->
    DepsList =
        case find_rebar_lock(Filename) of
            undefined -> [];
...
get_root_path({url, Path}) ->
    ...
    case Status of
        {ok, [{stdout, [Output]} | _]} ->
            #{path := Path2, sha256 := Sha256, rev := Rev} = jsone:decode(Output, [
                {keys, attempt_atom}
            ]),
            IsPrivate = string:find(Path, "ssh://", leading) =/= nomatch,
            #gitDep{
                name = binary_to_list(Path2),
                repo = Path,
...
get_root_path({file, File}) ->
    ...
    case filelib:is_dir(Path) of
        ...
        false ->
            io:format(
                "The path is not valid, invoke rebar32nix on the root folder of the project~n"
            ),
            erlang:halt(1)
    end;
get_root_path(UriOrFile) ->
    case uri_string:parse(UriOrFile) of
        #{scheme := "file", path := Path} ->
            get_root_path({file, Path});
        #{scheme := Scheme} ->
            case re:run(Scheme, "https?|git|ssh") of
                {match, _} ->
                    get_root_path({url, UriOrFile});
                _ ->
                    io:format(
                        "The URL uses an invalid protocol. Only http(s), git and ssh are supported~n"
                    )
            end;
        _ ->
            get_root_path({file, UriOrFile})
    end.
...
find_rebar_lock(Filename) ->
    ...
    case filelib:is_dir(Filename) of
        true ->
            find_rebar_lock(filename:absname_join(Filename, "./rebar.lock"));
        false ->
